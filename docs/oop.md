...
1. **Если родитель имеет `__slots__`**:  
   ...
   Нет необходимости явно включать слоты родителя (кроме случаев множественного наследования с конфликтующими слотами).
...
3. **Если потомок НЕ определяет свой `__slots__`**:  
   Он наследует только слоты родителя и **не получает `__dict__`** автоматически.

4. **Множественное наследование**:  
   Python автоматически объединяет `__slots__` разных родителей.  
   Если родители имеют одинаковые имена слотов — это нормально, слот считается общим.
...
| **2. Родитель БЕЗ `__slots__`**<br>Потомок СО `__slots__` | ❌ Нет | ❌ Только слоты потомка | `__dict__` не наследуется |
| **3. Родитель СО `__slots__`**<br>Потомок БЕЗ `__slots__` | ❌ Нет | ❌ Только унаследованные слоты | Слоты родителя |
| **4. Родитель СО `__slots__`**<br>Потомок СО `__slots__` | ❌ Нет | ❌ Только все слоты | Слоты объединяются автоматически |
| **5. Класс с `'__dict__'` в `__slots__`** | ✅ Да | ✅ Да | Явное разрешение `__dict__` |
...
print(hasattr(c, '__dict__'))  # ❌ False — нет словаря!
# c.y = 20  # ❌ AttributeError: нельзя создать новый атрибут
```
...
2. Наличие `__slots__` у родителя отключает `__dict__` для всей иерархии (если явно не восстановлен)
3. Явное объединение слотов рекомендуется для документации и особых случаев
4. Для восстановления динамических атрибутов добавьте `'__dict__'` в `__slots__`
