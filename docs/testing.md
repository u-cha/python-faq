...
## Что такое runtime в тестировании?

В тестировании ПО runtime testing (или runtime verification) — это проверка поведения приложения во время его выполнения (runtime), когда код уже запущен и работает в реальных условиях, а не в статическом анализе.​

Основные аспекты runtime в тестах:
* Мониторинг в реальном времени: отслеживание памяти, CPU, утечек, исключений и производительности во время работы (например, с помощью профайлеров или RASP — Runtime Application Self-Protection).​

* Динамическое обнаружение ошибок: выявление проблем, которые не видны на этапе компиляции/статического анализа, таких как race conditions, deadlock или переполнение стека.​

* Интеграция с CI/CD: автоматические тесты, запускаемые при деплое, проверяют приложение "в полевых условиях" (load testing, chaos engineering).​
...
def runtime_memory_test():
    process = psutil.Process()
    start_mem = process.memory_info().rss / 1024 / 1024  # MB
    # ... нагрузка ...
    end_mem = process.memory_info().rss / 1024 / 1024
    assert end_mem - start_mem < 100, "Утечка памяти!"
```
Отличается от compile-time (статические проверки до запуска) и unit-тестов (изолированные модули)

**Compile-time (время компиляции)**

Compile-time — это этап до запуска программы, когда код проверяется статически: синтаксис, типы (в строго типизированных языках), линтеры (flake8, mypy), статический анализ (pylint).​
Ошибки вроде SyntaxError или NameError ловятся здесь, без выполнения кода. В Python это минимально (интерпретируемый), но расширяется инструментами.
Пример: mypy script.py находит проблемы типизации до запуска.

**Unit-тесты (модульные тесты)**

Unit-тесты — это изолированные проверки отдельных функций/методов (с pytest, unittest), где моки (MagicMock) заменяют внешние зависимости (БД, API).​
Они запускаются в контролируемой среде, не в реальном приложении, и проверяют логику, а не интеграцию/производительность.
Пример: def test_add(): assert add(2, 3) == 5 — чистая функция без side-effects.

**Разница с runtime-тестированием**

Runtime-тесты работают во время выполнения полного приложения: мониторят реальные ресурсы (память, CPU), нагрузку, взаимодействия. Unit/compile-time не видят динамических проблем вроде утечек памяти или race conditions под нагрузкой