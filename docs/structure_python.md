...
Python относится к **типизированным языкам**. В отличие от ассемблера, где данные — просто набор битов, типизация позволяет интерпретатору обрабатывать данные по правилам, соответствующим их типу.

**Неявная (скрытая) типизация**

В Python тип переменной определяется автоматически при присваивании — **указывать его явно не нужно**:
...
Если бы типизация была **явной**, пришлось бы писать `int a = 1` — как в Java или C++.

**Аннотации типов (необязательные)**

С Python 3.5+ можно **добавлять подсказки типов**, но они не влияют на выполнение:

```python
def func(a: int, b: str) -> float:
    return round(float(a / len(b)), 2)

var: float = func(3, "123")  # IDE предупредит, если передадите список
```

Аннотации нужны для IDE и статических анализаторов (mypy), но **интерпретатор их игнорирует**.

**Сильная (строгая) типизация**

Python — язык с **строгой типизацией**. Это значит, что **нельзя смешивать несовместимые типы** без явного преобразования:

```python
2 + '2'  # ❌ TypeError: unsupported operand type(s) for +: 'int' and 'str'
```

В языках со **слабой типизацией** (например, JavaScript) такое сработало бы: `2 + '2'` → `'22'`.

**Но Python допускает некоторые исключения**

Python **разрешает** определенные преобразования, если они безопасны и естественны:

```python
2 + 2.2  # ✅ 4.2 (int + float → float)
```

**Динамическая типизация**

Python — язык с **динамической типизацией**. Тип привязан **к значению, а не к переменной**:

```python
a = 1        # int
a = 'строка' # str — можно!
a = SomeClass() # и даже объект
```

В языках со **статической типизацией** (C++, Java) переменная намертво привязана к своему типу.

**Плюсы динамической типизации:**

- Гибкость и скорость разработки
- Удобное обобщенное программирование

```python
def second(a):
    try:
        return a[1]  # работает с любым объектом, поддерживающим индексацию
    except TypeError:
        return None

second([1, 2])  # 2 (список)
second('abcd')  # 'b' (строка)
second(1)       # None (число не поддерживает)
```

**Минусы:**

- Более низкая скорость работы (типы проверяются во время выполнения)
- Ошибки с типами могут проявиться только при запуске программы

**Утиная типизация**

**"Если выглядит как утка, плавает как утка и крякает как утка — значит, это утка"**

В Python важен не тип объекта, а то, **какие методы он поддерживает**:
...
len('123')        # 3 (у строки есть __len__)
len([1, 2])       # 2 (у списка есть __len__)
len(SomeClass())  # 12 (у нашего класса тоже)
len(123)          # ❌ TypeError: object of type 'int' has no len()
```

Функция `len()` не проверяет тип — она просто вызывает магический метод `__len__()`. Если объект его реализует — работает, если нет — ошибка.

**Итог: типизация Python**

| Вид типизации | В Python | Что означает |
|---------------|----------|--------------|
| **По объявлению** | **Неявная** | Тип определяется автоматически, не нужно писать `int a = 1` |
| **По строгости** | **Сильная** | Нельзя сложить число со строкой без преобразования |
| **По времени** | **Динамическая** | Переменная может менять тип во время работы |
| **По поведению** | **Утиная** | Важны методы объекта, а не его формальный тип |

Гвидо ван Россум создавал Python как максимально понятный и удобный язык. Сочетание **неявной, динамической и утиной типизации** делает код лаконичным и выразительным, а **строгая типизация** защищает от многих ошибок, характерных для слаботипизированных языков.
...
**Проблема**: Этот механизм не может обработать циклические ссылки. Если два объекта (например, список и словарь) ссылаются друг на друга, но больше ни одна переменная в программе на них не указывает, их счётчики останутся равными 1. Они станут "мусором", но память занята навсегда.

**2. Циклический сборщик мусора с поколенческой оптимизацией**

Алгоритм работы (поиск циклов):

Garbage Collector работает с объектами-контейнерами (те, которые могут содержать ссылки на другие объекты: списки, словари, классы, кортежи). Алгоритм основан на отслеживании достижимости от корней (глобальных переменных, стека вызовов).

1. **Сбор**: Сборщик получает список всех объектов-контейнеров в проверяемой области памяти.
    
2. **Имитация удаления**: Он мысленно обрывает все ссылки внутри группы объектов и смотрит, у кого после этого останутся внешние связи.
    
3. **Пометка**: Те объекты, у которых остались ссылки извне (на них всё ещё кто-то ссылается за пределами возможного цикла), помечаются как "живые". Их ссылки восстанавливаются.
    
4. **Очистка**: Объекты, которые остались без внешних ссылок (чистый цикл), признаются мусором и удаляются.


**Оптимизация: Поколенческая сборка (Generational GC)**

Проверять все объекты в памяти при каждом запуске GC — слишком дорого. Поэтому применяется **эмпирическое правило: новые объекты умирают чаще, старые живут дольше (гипотеза поколений).**

Как это устроено (классическая схема до Python 3.13):
Объекты распределяются по трём поколениям в зависимости от их "возраста" (количества циклов GC, которые они пережили):

- **Поколение 0 (молодое)**: Сюда попадают все новые объекты. GC запускается здесь чаще всего.

- **Поколение 1 (среднее)**: Если объект пережил несколько сборок в поколении 0, он перемещается сюда. Проверяется реже.

- **Поколение 2 (старое)**: Самые "живучие" объекты. Проверяются очень редко.

GC автоматически запускается, когда количество вновь созданных объектов (в поколении 0) превышает пороговое значение (по умолчанию 700). После сборки поколения 0, оставшиеся в живых объекты перемещаются в поколение 1 и так далее.

**Новшества в Python 3.14+ : Инкрементальный сборщик мусора**

До версии 3.14 была проблема с так называемыми "Stop-the-World" паузами. Когда GC добирался до проверки старого поколения (2), он анализировал все объекты в памяти за один раз. Это могло вызывать заметные задержки в работе приложения (особенно в высоконагруженных или асинхронных сервисах).

Начиная с Python 3.14, классическая трёхпоколенческая схема заменена на инкрементальный (incremental) подход. От трёх поколений отказались в пользу более гибкой двухпоколенческой модели с разделением старой памяти на части (Молодое поколение (чистится всё сразу) и Старое поколение(чистится по частям).

**Суть инкрементальности**: Теперь полная сборка мусора не блокирует программу на всё время своего выполнения. Она разбивается на множество маленьких шагов (инкрементов), выполняемых между работой приложения.

**Результат**: Вместо одной долгой паузы (например, 500 мс) возникают множество коротких (например, по 5-10 мс), которые пользователь или система не замечают. Это критически важно для снижения задержек (latency) в real-time приложениях.

**Важные нюансы и подводные камни**

- Метод __del__: Если у объекта, участвующего в цикле, определён деструктор __del__, сборщик мусора не может безопасно определить порядок удаления объектов в цикле. В результате такие объекты не удаляются и попадают в специальный список gc.garbage, который программист должен обрабатывать вручную. Лучшая практика — избегать __del__ или использовать контекстные менеджеры (with).

- Модуль weakref (слабые ссылки): Основной инструмент для создания структур данных с циклическими связями (например, кэши, графы, где родитель должен ссылаться на потомка, а потомок — на родителя). Слабая ссылка (weakref.ref) не увеличивает счётчик ссылок, позволяя объекту корректно удалиться сборщиком мусора.

- Ручное управление: При необходимости можно управлять сборщиком вручную через модуль gc: отключать его (gc.disable()), запускать принудительно (gc.collect()) или настраивать пороги срабатывания.


**Ручное управление Garbage Collector (подробнее)**
...
Можно перечислить несколько блоков except для разных типов ошибок — каждый будет срабатывать только на свой тип исключения. Python проверяет их по порядку и выполняет первый подходящий блок. 
Порядок блоков имеет значение — сначала нужно обрабатывать конкретные типы ошибок, потом более общие.
